...
export function AttendancePage({ user, role, selectedWorkspace, groups, notify }: {
  user: User; role: ClubRole; selectedWorkspace: number | "club"; groups: Workspace[];
  notify: (message: string) => void;
}) {
  const canManage = role !== "member";
  const scope = selectedWorkspace === "club" ? "club" : "subgroup";
  const subgroupId = selectedWorkspace === "club" ? null : selectedWorkspace;
  const workspaceName = selectedWorkspace === "club" ? "Bharat Sangeet" : groups.find((group) => group.id === selectedWorkspace)?.name || "Subgroup";
  ...
  const load = useCallback(async (silent = false) => {
    ...
    let sessionQuery = supabase.from("attendance_sessions").select("id,scope,subgroup_id,title,starts_at,status,opened_at,closes_at,closed_at,canceled_at,code_expires_at,created_at,counts_toward_metrics").eq("scope", scope).order("starts_at", { ascending: false });
    sessionQuery = subgroupId === null ? sessionQuery.is("subgroup_id", null) : sessionQuery.eq("subgroup_id", subgroupId);
    const { data: sessionData, error: sessionError } = await sessionQuery;
    const nextSessions = (sessionData || []) as AttendanceSession[];
    ...
    setLoading(false);
  }, [canManage, notify, scope, subgroupId, user.id]);
  ...
  useEffect(() => { setTab("overview"); setLiveCode(""); setFeedback(""); }, [selectedWorkspace]);
  ...
  async function createSession(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await supabase.rpc("create_attendance_session", {
      p_scope: scope, p_subgroup_id: subgroupId, p_title: String(form.get("title") || "Meeting"),
  ...
  return <section className="section-shell page-section attendance-page">
    <div className="attendance-title-row"><div><span className="scope-badge">{scope === "club" ? "Club-wide" : "Subgroup"}</span><p className="eyebrow">{workspaceName.toUpperCase()}</p><h1>Attendance</h1><p>Live meeting check-in, attendance history, and absence support for this workspace.</p></div>{canManage && <button className="primary" onClick={() => setShowNewSession(true)}>＋ New attendance session</button>}</div>

    <form className="attendance-checkin" onSubmit={checkIn}><div><p className="eyebrow">MEMBER CHECK-IN</p><h2>Enter the meeting code</h2><p>Use the code shown by an executive while check-in is open.</p></div><div><input value={checkInCode} onChange={(event) => setCheckInCode(event.target.value)} required minLength={6} maxLength={8} inputMode="numeric" autoComplete="one-time-code" placeholder="000000" aria-label="Attendance code" /><button className="primary" disabled={saving}>{saving ? "Checking…" : "Check in"}</button></div>{feedback && <p className={`attendance-feedback ${feedback.includes("confirmed") ? "success" : "error"}`} role="status">{feedback}</p>}</form>
    ...
    {canManage ? <>
      ...
      <div className="member-attendance-grid"><div className="attendance-panel"><div className="attendance-panel-heading"><div><p className="eyebrow">MY HISTORY</p><h2>Meeting attendance</h2></div><span>{workspaceName}</span></div><div className="member-history">{sessions.length ? sessions.filter((session) => session.status !== "draft").map((session) => { const record = ownRecords.find((item) => item.session_id === session.id); return <article key={session.id}><div><b>{session.title}</b><small>{formatDateTime(session.starts_at)}</small></div><span className={`attendance-status ${record?.status || session.status}`}>{record?.status || session.status}</span></article>; }) : <div className="attendance-empty">No meetings are available in this workspace.</div>}</div></div><div className="attendance-panel"><div className="attendance-panel-heading"><div><p className="eyebrow">ABSENCE SUPPORT</p><h2>Need an excuse?</h2></div></div><p className="attendance-help">Send executives the meeting, your reason, and a private proof image. Only you and executives can view it.</p><button className="primary" onClick={() => setShowExcuse(true)} disabled={!sessions.some((session) => session.status !== "canceled" && session.status !== "draft")}>Submit absence excuse</button><div className="member-excuses">{excuses.map((excuse) => <article key={excuse.id}><div><b>{sessions.find((session) => session.id === excuse.session_id)?.title || "Meeting"}</b><small>{formatDateTime(excuse.submitted_at)}</small></div><span className={`attendance-status ${excuse.status}`}>{excuse.status}</span></article>)}</div></div></div>
    </>}

    {showNewSession && <div className="modal-backdrop" onMouseDown={() => setShowNewSession(false)}><form className="modal attendance-modal" onSubmit={createSession} onMouseDown={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setShowNewSession(false)}>×</button><p className="eyebrow">{workspaceName.toUpperCase()}</p><h2>New attendance session</h2><p className="modal-copy">The current workspace determines who is eligible to check in.</p><label>Meeting name<input name="title" required placeholder="Weekly rehearsal" /></label><label>Meeting date and time<input name="starts_at" type="datetime-local" required defaultValue={localDateTime()} /></label><label>Code duration<select name="duration" defaultValue="60"><option value="15">15 minutes</option><option value="30">30 minutes</option><option value="60">1 hour</option><option value="120">2 hours</option></select></label><label className="check-label"><input name="open_now" type="checkbox" defaultChecked /> Open check-in immediately</label><button className="primary" disabled={saving}>{saving ? "Creating…" : "Create session"}</button></form></div>}
    {showExcuse && <div className="modal-backdrop" onMouseDown={() => setShowExcuse(false)}><form className="modal attendance-modal" onSubmit={submitExcuse} onMouseDown={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setShowExcuse(false)}>×</button><p className="eyebrow">PRIVATE SUBMISSION</p><h2>Submit an absence excuse</h2><p className="modal-copy">Your reason and proof are visible only to you and club executives.</p><label>Meeting<select name="session_id" required defaultValue=""><option value="" disabled>Choose a meeting</option>{sessions.filter((session) => session.status !== "canceled" && session.status !== "draft").map((session) => <option value={session.id} key={session.id}>{session.title} — {formatDateTime(session.starts_at)}</option>)}</select></label><label>Reason<textarea name="reason" required rows={5} minLength={10} placeholder="Explain why you could not attend…" /></label><label>Picture for proof<input name="proof" type="file" required accept="image/*" /><small>JPG, PNG, HEIC, or another image up to 8 MB.</small></label><button className="primary" disabled={saving}>{saving ? "Submitting…" : "Submit excuse"}</button></form></div>}
  </section>;
}
