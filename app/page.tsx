...
export default function Home() {
  ...
  const [uploadType, setUploadType] = useState<ArchiveType | "financial">("recording");
  ...
  const navigate = (next: Section) => {
    ...
    else if (next === "finances") setUploadType("financial");
    if (next === "home" && user) loadData(role);
    setSection(next);
  ...
  return (
    <main>
      ...
      {section === "finances" && canManage && <section className="section-shell page-section"><PageTitle eyebrow="EXECUTIVE ACCESS" title="Club finances" text="A private, clear record of the funds that support our music." scope="Club-wide" /><div className="finance-summary"><div><small>AVAILABLE BALANCE</small><strong>{money(balance)}</strong><span>Live club ledger</span></div><div><small>TOTAL INCOME</small><b className="income">{money(income)}</b><span>Recorded funds</span></div><div><small>TOTAL EXPENSES</small><b>{money(expenses)}</b><span>Recorded spending</span></div><button className="primary" onClick={() => { setUploadType("financial"); setShowUpload(true); }}>＋ Add transaction</button></div>{transactions.length ? <div className="ledger"><div className="ledger-head"><b>Recent activity</b><span>Club ledger</span></div>{transactions.map((item) => <div className="ledger-row" key={item.id}><span className={`money-mark ${Number(item.amount) > 0 ? "income" : "expense"}`}>{Number(item.amount) > 0 ? "+" : "−"}</span><b>{item.description}</b><span>{formatDate(item.transaction_date)}</span><strong className={Number(item.amount) > 0 ? "income" : "expense"}>{money(Number(item.amount))}</strong></div>)}</div> : <EmptyState title="No transactions yet" text="Add the first allocation, donation, or expense." />}</section>}
      ...
      {showUpload && <ArchiveModal user={user} initialType={uploadType} onClose={() => setShowUpload(false)} onSaved={() => { setShowUpload(false); loadData(role); notify("Saved to the club archive"); }} notify={notify} />}
...
function PortalPageMenu({ section, role, onNavigate }: { section: Section; role: ClubRole; onNavigate: (section: Section) => void }) {
  ...
  const managePages: Section[] = [...(role !== "member" ? ["finances" as Section] : []), ...(role === "admin" ? ["admin" as Section] : [])];
...
function ArchiveModal({ user, initialType = "recording", onClose, onSaved, notify }: { user: User; initialType?: ArchiveType | "financial"; onClose: () => void; onSaved: () => void; notify: (message: string) => void }) {
  const [type, setType] = useState<ArchiveType | "financial">(initialType);
  const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!supabase) return; setSaving(true);
